...
<body>
    <div class="container">
    <h4 style="color: #029ed6; text-align: left;" class="text-uppercase"><i class="fa-solid fa-bell"></i> AT&T MASTER ALARM LIST</h4>
            ...
            <?php
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo '<td><span class="' . $badgeClass . '">' . htmlspecialchars($row["severity"]) . '</span></td>';
                        echo "<td>" . $row["normallyopen"] . "</td>";
                        echo '<td><button class="btn btn-warning btn-sm" onclick="editRecord(\'' . $row["port"] . '\', \'' . $row["slogan"] . '\', \'' . $row["severity"] . '\', \'' . $row["normallyopen"] . '\')"><i class="fa-solid fa-pen-to-square"></i></button></td>';
                        echo "</tr>";
                    }
                } else {
        ...
        <h4 style="color: #029ed6; text-align: left;" class="text-uppercase"><i class="fa-solid fa-pen-to-square"></i> Edit Alarm</h4>
        <form method="post" action="">
        <div class="form-group">
            <label for="port">Port:</label>
            <input type="text" id="port" name="port" readonly class="form-control">
        </div>
        <div class="form-group">
            <label for="slogan">Slogan:</label>
            <input type="text" id="slogan" name="slogan" required class="form-control">
        </div>
        <div class="form-group">
            <label for="severity">Severity:</label>
            <input type="text" id="severity" name="severity" required class="form-control">
        </div>
        <div class="form-group">
            <label for="normallyopen">Normally Open:</label>
            <input type="text" id="normallyopen" name="normallyopen" required class="form-control">
        </div>
        <?php if (!empty($message)) { ?>
            <div class="alert alert-success" role="alert" <?php echo strpos($message, 'successfully') !== false ? 'success' : 'error'; ?>">
            <i class="fa-solid fa-circle-check"></i> <?php echo $message; ?>
            </div>
        <?php } ?>
            <button type="submit" class="btn btn-info">Update <i class="fa-solid fa-upload"></i></button>
        </form>
    </div>
